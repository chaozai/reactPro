...
/**
 * Stores historic data about a VM.存储关于VM的历史数据�??
 * 
...
public class VmStateHistoryEntry {
	 ...
	 */
	public VmStateHistoryEntry(double time, double allocatedMips, double requestedMips, double currentWrongProbability, boolean isInMigration) {
		setTime(time);
		setAllocatedMips(allocatedMips);
		setRequestedMips(requestedMips);
		setCurrentWrongProbability(currentWrongProbability);
