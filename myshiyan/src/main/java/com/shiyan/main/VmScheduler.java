 ...
 * 
 * Each host has to use is own instance of a VmScheduler 每个主机必须使用自己的VmScheduler实例
 * that will so schedule the allocation of host's PEs for VMs running on it.
 * 这样就可以安排主机上运行的vm的PEs分配
...
public abstract class VmScheduler {
	...
	/** The total available MIPS that can be allocated on demand for VMs.可以根据vm的需求分配的可用MIPS总数 */
	...
	public abstract int getVmsMigrationSize ();
