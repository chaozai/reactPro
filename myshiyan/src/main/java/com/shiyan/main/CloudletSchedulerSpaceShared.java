...
public class CloudletSchedulerSpaceShared extends CloudletScheduler {
	/** The number of PEs currently available for the VM using the scheduler,使用调度程序的VM当前可用的PEs的数量
         * according to the mips share provided to it by 根据mips提供给它的份额，通过updateVmProcessing方法。
    ...
    * {@link #updateVmProcessing(double, java.util.List)} method. */
	 ...
	 * 创建一个新的cloudletscheduling erspaceshared对象。在此之前必须调用此方法开始实际的模拟
	 * @pre $none
	 * @post $none
	 */
	public CloudletSchedulerSpaceShared() {
		...
		setCurrentWrongProbability(0.0);
	}
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		double capacity = 0.0;//cpu容量
		int cpus = 0;

		for (Double mips : mipsShare) { // count the CPUs available to the VMM 计算VMM可用的cpu数量
			capacity += mips;
			if (mips > 0) {
				cpus++;
			}
		...
		currentCpus = cpus;
		capacity /= cpus; // average capacity of each cpu 每个cpu的平均容量

		// each machine in the exec list has the same amount of cpu// exec列表中的每台计算机的cpu数量都相同
		...
		// for each finished cloudlet, add a new one from the waiting list对于每个完成的cloudlet，从等待列表中添加一个新的云任务去执行
		...
		// estimate finish time of cloudlets in the execution queue 估计执行队列中cloudlets的最短完成时间
		double nextEvent = Double.MAX_VALUE;
		for (ResCloudlet rcl : getCloudletExecList()) {//得到执行列表中任务的最小执行时间
			double remainingLength = rcl.getRemainingCloudletLength();
			double estimatedFinishTime = currentTime + (remainingLength / (capacity * rcl.getNumberOfPes()));
			if (estimatedFinishTime - currentTime < CloudSim.getMinTimeBetweenEvents()) {//任务执行时间过小，则忽略此任务的执行时间
	...
	public Cloudlet cloudletCancel(int cloudletId) {
		// First, looks in the finished queue 首先，查看完成的队列
		...
		// Then searches in the exec list 然后在执行列表中搜索
		for (ResCloudlet rcl : getCloudletExecList()) {
			if (rcl.getCloudletId() == cloudletId) {
		...
		// Now, looks in the paused queue 现在,看一下暂停排队
		...
		// Finally, looks in the waiting list 最后,看一下等待列表
		for (ResCloudlet rcl : getCloudletWaitingList()) {
			if (rcl.getCloudletId() == cloudletId) {
				rcl.setCloudletStatus(Cloudlet.CANCELED);
	...
	public boolean cloudletPause(int cloudletId) {
		...
		// first, looks for the cloudlet in the exec list 在执行列表中查找cloudlet
		for (ResCloudlet rcl : getCloudletExecList()) {
			if (rcl.getCloudletId() == cloudletId) {
				found = true;
				break;
		...
		if (found) {
			// moves to the paused list 移动到暂停列表
	...
	@Override//从暂停中，重新开始，恢复运行
	public double cloudletResume(int cloudletId) {
		boolean found = false;
		int position = 0;

		// First, look for the cloudlet in the paused list
		for (ResCloudlet rcl : getCloudletPausedList()) {
			if (rcl.getCloudletId() == cloudletId) {
				found = true;
				break;
	...
	public double cloudletSubmit(Cloudlet cloudlet, double fileTransferTime) {
		// it can go to the exec list 云任务提交时，它可以进入执行列表 ，但是，首先要判断该虚拟机是否有足够的pe若没有足够的pe则把任务添加到等待列表
		...
		} else {// no enough free PEs: go to the waiting queue 若没有闲置的pes时:添加到等待队列
			ResCloudlet rcl = new ResCloudlet(cloudlet);
			rcl.setCloudletStatus(Cloudlet.QUEUED);
			getCloudletWaitingList().add(rcl);
			return 0.0;
		...
		// calculate the expected time for cloudlet completion 计算cloudlet的预期完成时间
		double capacity = 0.0;
		int cpus = 0;
		for (Double mips : getCurrentMipsShare()) {
			capacity += mips;
	...
	public double getTotalUtilizationOfRam(double time) {
		...
		for (ResCloudlet gl : getCloudletExecList()) {
			totalRamUtilization += gl.getCloudlet().getUtilizationOfRam(time);
		}
		return totalRamUtilization;
	}
	...
	@Override
	public double cloudletExpectedExecTime(Cloudlet cloudlet, double fileTransferTime) {
