...
public class Cloudlet {
    ...
    /**
     * The input file size of this Cloudlet before execution (unit: in byte).执行之前此Cloudlet的输入文件大�?
     * This size has to be considered the program + input data sizes.这个大小必须考虑程序+输入数据大小
     */
    private final long cloudletFileSize;

    /**
     * The output file size of this Cloudlet after execution (unit: in byte).
            *   执行之后这个Cloudlet的输出文件大�?
     * @todo See
     ...
     * execution time. Previous execution time are ignored.
             * 这个Cloudlet的执行开始时间�?�有了新的功能当取消、暂停和恢复时，此属性只存储�?新的执行时间�?
             * 忽略先前的执行时�?
     */
    private double execStartTime;

    /**
     * The time where this Cloudlet completes.这个云任务完成时�?
     */
    private double finishTime;

    /**
     * The ID of a reservation made for this cloudlet.此cloudlet预订的ID
     *
     * @todo This attribute doesn't appear to be used 此属性似乎未被使�?
     */
    private int reservationId = -1;

    /**
     * Indicates if transaction history records for this Cloudlet is to be
     * outputted.指示此Cloudlet的事务历史记录是否为输出�?
     */
    private final boolean record;

    /**
     * Stores the operating system line separator.存储操作系统行分隔符�?
     */
    private String newline;

    /**
     * The cloudlet transaction history.cloudlet事务历史记录
     */
    private StringBuffer history;

    /**
     * The list of every resource where the cloudlet has been executed. In case执行cloudlet的每个资源的列表。以防它在单个云资源中启动和结束执行
     * it starts and finishes executing in a single cloud resource, without在迁移过程中，这个列表将只有
     * being migrated, this list will have only one item.
     */
    private final List<Resource> resList;

    /**
     * The index of the last resource where the cloudlet was executed. If the执行cloudlet的最后一个资源的索引�?
     * cloudlet is migrated during its execution, this index is updated. The如果cloudlet在执行期间被迁移，这个索引被更新
     * value -1 indicates the cloudlet has not been executed 当为-1时，表示该云任务还没有被执行
         yet.
     */
    private int index;
     ...
     * as planned. 
     * Cloudlet已分配给按原计划执行的CloudResource对象*
     */
    public static final int READY = 1;

    /**
     * The Cloudlet has moved to a Cloud node.Cloudlet 已经移动到一个云节点
     */
    public static final int QUEUED = 2;

    /**
     * The Cloudlet is in execution in a Cloud node.Cloudlet正在云节点中执行
     */
    public static final int INEXEC = 3;

    /**
     * The Cloudlet has been executed successfully.云任务已经被成功执行
     */
    public static final int SUCCESS = 4;

    /**
     * The Cloudlet has failed.云任务失�?
     */
    public static final int FAILED = 5;

    /**
     * The Cloudlet has been canceled.云任务被取消
     */
    ...
    /**
     * The cloudlet has failed due to a resource failure.由于资源故障，cloudlet已经失败�?
     */
    ...
    /**
     * The cost of each byte of bandwidth (bw) consumed.消�?�的每个字节的带宽成�?(bw)
     */
    protected double costPerBw;

    /**
     * The total bandwidth (bw) cost for transferring the cloudlet by the根据cloudletFileSize�?得的通过网络迁移云任务的总花�?
     ...
     * CPU.定义云任务如何使用vm的cpu的利用率模型
     */
    private UtilizationModel utilizationModelCpu;

    /**
     * The utilization model that defines how the cloudlet will use the VM's
     * RAM.定义云任务如何使用vm的Ram的利用率模型
     */
    private UtilizationModel utilizationModelRam;

    /**
     * The utilization model that defines how the cloudlet will use the VM's
     * bandwidth (bw).定义云任务如何使用vm的bw的利用率模型
     */
    private UtilizationModel utilizationModelBw;

    // Data cloudlet
    /**
     * The required files to be used by the cloudlet (if any). The time tocloudlet�?要使用的文件(如果有的�?)
     * transfer these files by the network is considered when placing the时间
            *在放置时，为给定VM中的cloudlet考虑通过网络传输这些文件 
     ...
     * 获取迄今为止已执行的此Cloudlet的长度
     * @return the length of a partially executed Cloudlet or the full Cloudlet
     * length if it is completed 部分执行的Cloudlet的长度，如果已完成，则为完整的Cloudlet长度
     * @pre $none
     * @post $result >= 0.0
     */
    ...
    public void setResourceParameter(final int resourceID, final double cost) {
        ...
        // add into a list if moving to a new grid resource如果移动到新的网格资源，则添加到列表�?
        resList.add(res);
     ...
     * 将此Cloudlet的提交(到达)时间设置为CloudResource
     ...
     * executed. 为这个即将运行的云任务设置云资源参数<br>
     * NOTE: This method <tt>should</tt> be called only by a resource entity,
     * not the user or owner of this Cloudlet.
     *
     * @param resourceID the CloudResource ID
	...
	public double getStartSubmit() {
		return startSubmit;
	}

	public void setStartSubmit(double startSubmit) {
