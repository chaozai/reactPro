...
public class ResCloudlet {
	 ...
	 * can be canceled, paused or resumed.Cloudlet执行开始时间。这个属性只包含Cloudlet之后的最新时间
	 */
	...
	private void init() {
		...
		// if more than 1 Pe, then create an array如果超过1个Pe，则创建一个数组
		if (pesNumber > 1) {
		...
		// hostList.如果某个Cloudlet被其他网格主机列表部分执行
