...
import com.shiyan.core.predicates.PredicateAny;
import com.shiyan.core.predicates.PredicateNone;
...
public class CloudSim {
	...
	/** The id of CloudSimShutdown entity. CloudSim关闭实体的id*/
	...
	/** The Constant NOT_FOUND. 常数NOT_FOUND*/
	...
	/** The minimal time between events. Events within shorter periods after the last event are discarded.事件之间的最短时间。最后一个事件之后较短时间内的事件被丢弃 */
	private static double minTimeBetweenEvents = 0.1;
	
	/**
	 * Initialises all the common attributes.初始化所有公共属性
	 * 
	 * @param _calendar the _calendar
	 * @param _traceFlag the _trace flag
	 * @param numUser number of users
	 * @throws Exception This happens when creating this entity before initialising CloudSim package这是在初始化CloudSim包之前创建这个实体时发生的
	 ...
	 * <p>初始化cludsim参数，这个方法在创建任何实体之前被调用
	...
	public static void init(int numUser, Calendar cal, boolean traceFlag) {
		try {
			...
			// create a GIS object 创建一个GIS对象
			cis = new CloudInformationService("CloudInformationService");

			// set all the above entity IDs 设置上述实体的id
			cisId = cis.getId();
	...
	/**
	 * Initialises CloudSim parameters. This method should be called before creating any entities. 初始化CloudSim参数。应该在创建任何实体之前调用此方法
	 * <p>
	 * Inside this method, it will create the following CloudSim entities: 在这个方法中，它将创建以下CloudSim实体
	 * <ul>
	 * <li>CloudInformationService. 云信息服务
	 * <li>CloudSimShutdown   云平台关闭
	 * </ul>
	 * <p>
	 * 
	 * @param numUser the number of User Entities created. This parameters indicates that
	 ...
	 *            taken from <tt>Calendar.getInstance()</tt>
	 * @param traceFlag <tt>true</tt> if CloudSim trace need to be written
	 * @param periodBetweenEvents - the minimal period between events. Events within shorter periods 事件之间的最小周期，最后一个事件之后较短时间内的事件被丢弃
	...
	/**
	 * Starts the execution of CloudSim simulation. It waits for complete execution of all entities,开始执行CloudSim模拟。当所有实体都被执行
	 * i.e. until all entities threads reach non-RUNNABLE state or there are no more events in the 或者直到所有实体线程达到不可运行状态，
	 * future event queue. 或者在将来的事件队列中没有更多的事件
	 * <p>
	 * <b>Note</b>: This method should be called after all the entities have been setup and added. 在设置和添加所有实体之后，应该调用此方法
	 * 
	 * @return the last clock time 最后一个时钟时间
	...
	public static double startSimulation() throws NullPointerException {
		...
		try {
			...
			// reset all static variables 重置所有静态变量
			cisId = -1;
			shutdownId = -1;
			cis = null;
			calendar = null;
	 ...
	 * any of the user defined entities <b>explicitly</b> want to terminate simulation during 用户在执行期间终止模拟则调用此方法
	 * execution.
	 * 
	 * @throws NullPointerException This happens when creating this entity before initialising
	 *             CloudSim package or this entity name is <tt>null</tt> or empty
	...
	/**
	 * This method is called if one wants to terminate the simulation.如果一个人想要终止仿真,就会调用这个方法
	 * 
	...
	/**
	 * This method is called if one wants to terminate the simulation at a given time. 如果一个人想在给定的时间终止模拟,该方法被调用
	 * 
	...
	/**
	 * Returns the minimum time between events. Events within shorter periods after the last event are discarded. 返回事件之间的最小时间。在最后一事件结束后的短时间内事件被丢弃。
	...
	/**
	 * Gets a new copy of initial simulation Calendar.获取初始模拟日历的新副本
	 * 
	...
	/**
	 * Gets the entity ID of <tt>CloudInformationService</tt>.得到云信息中的实体id
	 * 
	 ...
	 * hostList.向云信息服务(CIS)实体发送请求,以获取所有云的主机列表。
	 * 
	...
	/** The future event queue. 未来事件队列*/
	protected static FutureQueue future;

	/** The deferred event queue. 延迟事件队列*/
	protected static DeferredQueue deferred;

	/** 
         * The current simulation clock.当前仿真时钟
         */
	private static double clock;

	/** Flag for checking if the simulation is running. 用于检查模拟是否正在运行的标志*/
	private static boolean running;

	/** The entities by name. 名称实体群映射*/
	private static Map<String, SimEntity> entitiesByName;

	// The predicates used in entity wait methods
	/** The wait predicates. 等待谓词*/
	 ...
	 * start of the simulation.初始化独立模拟,这个函数应该在模拟开始时被调用。
	 */
	...
	/** A standard predicate that matches any event.匹配任何事件的标准谓词 */
	public final static PredicateAny SIM_ANY = new PredicateAny();

	/** A standard predicate that does not match any events. 没有匹配任何事件的标准谓词*/
	public final static PredicateNone SIM_NONE = new PredicateNone();

	// Public access methods 公共存取方法

	/**
	 * Get the current simulation time. 得到当前的模拟时间
	 * 
	...
	/**
	 * Get the current number of entities in the simulation. 获取当前模拟中实体的数量
	 * 
	...
	/**
	 * Get the entity with a given id. 用给定的id获取实体
	 * 
	...
	/**
	 * Get the entity with a given name.以给定的名称获取实体
	 * 
	...
	/**
	 * Get the id of an entity with a given name. 获取给定名称的实体的id
	 * 
	 * @param name The entity's name
	...
	/**
	 * Gets name of the entity given its entity ID. 通过实体ID获取实体的名称
	 * 
	...
	/**
	 * Returns a list of entities created for the simulation. 返回为模拟创建(simulation)的实体列表
	 * 
	 ...
	 * simulations since entities are automatically added to the simulation upon instantiation.
	   *   在模拟中添加一个新的实体，这是为了与现有的模拟兼容的，因为实体在实例化时自动添加到仿真中。
	 * @param e The new entity 新的实体
	 */
	 ...
	 * should <b>not</b> be called from user simulations.在仿真运行时,用于动态向仿真添加一个新实体的内部方法，它应该由用户模拟中调用
	 * 
	 ...
	 * called in simulations.内部方法用于运行一个标记的模拟
	 * 
	 * @return true, if successful otherwise
	...
	public static boolean runClockTick() {
		...
		for (int i = 0; i < entities_size; i++) {//执行延迟队列中的事件
			ent = entities.get(i);
		...
		// If there are more future events then deal with them 如果有更多的未来事件，那么就处理它们
		if (future.size() > 0) {
			List<SimEvent> toRemove = new ArrayList<SimEvent>();
			Iterator<SimEvent> fit = future.iterator();
			queue_empty = false;
			SimEvent first = fit.next();
			processEvent(first);//将事件加入到延迟队列
			future.remove(first);

			fit = future.iterator();

			// Check if next events are at same time...检查下一个事件是否同时发生
			boolean trymore = fit.hasNext();//是否有下一个事件
			while (trymore) {
	...
	/**
	 * Internal method used to stop the simulation. This method should <b>not</b> be used directly.内部方法用于停止模拟
	 */
	...
	/**
	 * Used to hold an entity for some time.用于持有一个实体一段时间
	 * 
	 * @param src the src
	...
	/**
	 * Used to pause an entity for some time. 用来暂停一个实体一段时间。
	 * 
	 * @param src the src
	 * @param delay the delay
	 */
	...
	/**
	 * Used to send an event from one entity to another.用于将事件从一个实体发送到另一个实体
	 * 
	 * @param src the src
	 * @param dest the dest
	 * @param delay the delay
	...
	public static boolean cancelAll(int src, Predicate p) {
		SimEvent ev = null;
		int previousSize = future.size();//之前的大小
		Iterator<SimEvent> iter = future.iterator();
		while (iter.hasNext()) {//如果迭代有更多的元素，返回true
			ev = iter.next();
			if (ev.getSource() == src && p.match(ev)) {
				iter.remove();
			}
	...
	private static void processEvent(SimEvent e) {
		...
		switch (e.getType()) {
			...
			case SimEvent.SEND:
				// Check for matching wait 检查匹配等待
	 ...
	 * simulations.内部方法用于启动仿真。这种方法不应该用于用户模拟
	 */
	 ...
	 * and added, and their ports linked.启动模拟运行。这应该在所有实体被设置和添加，以及它们的端口被链接之后调用
	 * 
	 * @return the last clock value 返回最后一个时钟值
	 */
	public static double run() {
		if (!running) {
			runStart();//创建每个实体的SimeEvevt事件，并将其加入到future队列中
		}
		while (true) {
			if (runClockTick() || abruptTerminate) {//runClockTick()方法
				break;
			}

			// this block allows termination of simulation at a specific time此块允许在特定时间终止模拟
	 ...
	 * in user simulations.允许实体终止的内部方法
	 */
	...
	/**
	 * Abruptally terminate. Abruptally终止
	 */
	...
	/**
	 * Prints a message about the progress of the simulation. 打印关于模拟进展的消息
	 * 
